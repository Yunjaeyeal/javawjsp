...
create table member (
	...
	pwd varchar(100) not null,         /*비밀번호(SHA암호화 처리)  100개주는이유 암호화된값이 엄청큼*/
	...
	job varchar(20),                  /* 회원직업 */
	hobby varchar(100),                /* 회원취미 (2개이상은 '/'로 구분처리) */
	...
	point int default 100,            /* 회원누적포인트 (가입시는 기본 100포인트 증정,방문시마다 1회 10포인트증가, 최대 1일 50포인트까지) */
	level int default 1,              /*회원등급(0:관리자, 1:준회원 ,2:정회원, 3:우수회원, (4:운영자) */	
	visitCnt int default 0,           /*방문횟수*/
	startDate datetime default now(), /*최초 가입일*/
	lastDate datetime default now(), /*마지막 접속일*/
	todayCnt int default 0,           /*오늘 방문한 횟수*/
	primary key(idx,mid)                 /* 주키 : idx(고유번호) , mid(아이디)<중복아이디방지 */
);
...
insert into member values (default,'admin','1234','관리맨','관리자',default,default,'010-3423-3704','청주 금천동','woduffld123@naver.com','http://woduf1234.tistory.com','프리랜서','축구/야구',default,'관리자입니다',default,default,default,0,default,default,default,default);