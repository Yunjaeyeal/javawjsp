...
insert into board values (default,'관리맨','게시판서비스를 시작합니다','woduf1234@naver.com','woduf1234.tistory.com','이곳은 게시판입니다',default,'192.168.50.144',default,default,'admin');
...
select year(now());   /* year 년도출력 */
select month(now());  /* month 월출력 */
select day(now());    /* day 일출력 */
select date(now());   /* date 년-월-일 출력 */
select weekday(now()); /* 요일 : 0(월) 1(화) 2(수) 3(목) 4(금) 5(토) */ 
...
select weekday('2022-12-13');
select idx,weekday(wDate) from board;
...
select from_days('2022-12-5');
--년도(2자리):%y 년도(4자리):%Y ,월:%m ,월(영문):%M ,일:%d ,시:%h ,분%i, 초%s
...
select date_sub(wDate,interval 1 day) from board;
...
--board테이블의 고유번호 22번글에 딸려있는 댓글의 개수는?
select count(*) from boardReply where boardIdx = 22; 

--board테이블의 고유번호 22번글에 딸려있는 댓글의 개수는+원본글의 고유번호와 함께 출력 갯수의별명은 replyCnt
select boardIdx, count(*) as replyCnt from boardReply where boardIdx = 22; 

--board테이블의 고유번호 22번글에 딸려있는 댓글의 개수는+원본글의 고유번호와 함께 출력 갯수의별명은 replyCnt+ 단,닉네임은 board(원본글)에서 가져와서 출력하시오.
...
count(*) AS replyCnt 
FROM boardReply WHERE boardIdx = 22; 
...
-- 부모관점(board)에서 고유번호 22번의 아이디와,현재글에 달려있는 댓글의 갯수
SELECT mid,
(SELECT count(*) FROM boardReply WHERE boardIdx=22 ) as replyCnt 
FROM board WHERE idx =22; 
-- 부모관점(board)에서 board테이블의 모든 내용과,현재글에 달려있는 댓글의 갯수를 가져오되, 최근글 5개만 출력
SELECT *,
...
select * from board where idx > 5 order by idx  limit 1;
