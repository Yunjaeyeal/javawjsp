...
public class Test2_4 extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 서버에서 get방식으로의 전송방법
//		response.sendRedirect(request.getContextPath()+"/study/1113/test2Ok3.jsp?hakbun="+hakbun+"&name="+name+"&kor="+kor+"&eng="+eng+"&mat="+mat+"&soc="+soc+"&sci="+sci+"&tot="+tot+"&avg="+avg+"&grade="+grade);
//		response.sendRedirect(request.getContextPath()+"/study/1113/test2Ok3.jsp?vo="+vo);
		
		// 저장소(request)를 활용한 forwarding
		request.setAttribute("vo", vo); // 뒤에 잇는 변수는 객체변수 , 앞에잇는변수는 객체를 담아갈 변수
//		RequestDispatcher dispatcher= request.getRequestDispatcher("/study/1113/test2Ok4_2.jsp"); // ()안에 forwarding 할 주소적어줌
//		dispatcher.forward(request, response);
		
		String viewpage = "/study/1113/test2Ok4_2.jsp";
